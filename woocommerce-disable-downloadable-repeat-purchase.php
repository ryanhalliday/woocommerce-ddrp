...
function som_disable_repeat_purchase( $purchasable, $product ) {
	...
	if ($product->downloadable == 'yes' && wc_customer_bought_product( wp_get_current_user()->user_email, get_current_user_id(), $product_id ) ) {
		global $som_user_dl_permissions;
...
function som_download_buttons() {
	...
	if ( wc_customer_bought_product( wp_get_current_user()->user_email, get_current_user_id(), $product->get_id() ) && ($product->downloadable == 'yes') ) {
		...
		$product_download_permissions = array_filter($som_user_dl_permissions, function($permission) use ($product){
			return $permission->product_id == $product->id;
		});
		...
		foreach ( $product->get_downloads() as $download ){
			...
			$download_url = som_get_download_url($product->id, $download['id'], $product_download_permissions);
